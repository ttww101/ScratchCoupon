...
public class JXMarqueeView: UIView {
    ...
    override open func layoutSubviews() {
        ...
        }else if marqueeType == .up {
            containerView.frame = CGRect(x: 0, y: 0, width: validContentView.bounds.size.width, height: self.bounds.size.height*2 + contentMargin)
        }else {
        ...
        if marqueeType == .up {
            if validContentView.bounds.size.height > self.bounds.size.height {
